...
with tabs[0]:  
    ...
    if lap_data:  
        if playback:  
            ...
            playback_data = [l for l in lap_data if l['lap_number'] == st.session_state.playback_lap]  
            if playback_data:  
                car_positions = pd.DataFrame({  
                    "x": [p['x'] for p in playback_data if 'x' in p],  
                    "y": [p['y'] for p in playback_data if 'y' in p],  
                    "driver": [p['driver_number'] for p in playback_data if 'x' in p]  
                })  
        else:  
            # Use latest positions  
            car_positions = pd.DataFrame({  
                "x": [l['x'] for l in lap_data if 'x' in l],  
                "y": [l['y'] for l in lap_data if 'y' in l],  
                "driver": [l['driver_number'] for l in lap_data if 'x' in l]  
            })  
...
with tabs[2]:  
    ...
    if driver_data and lap_data:  
        ...
        # Add team names  
        standings_df["Team"] = [next((d['team_name'] for d in driver_data if d['driver_number'] == dr), "N/A") for dr in standings_df["Driver Number"]]  
...
with tabs[4]:  
    ...
    st.number_input("Refresh Interval (seconds)", min_value=5, max_value=60, value=5)  
